import React from 'react';
import { ScrollView, View } from 'react-native';
import { Appbar, List, Button, IconButton, Text } from 'react-native-paper';
import { useCart } from '../context/CartContext';
import tw from 'twrnc';

export default function CarrinhoScreen({ navigation }) {
  const { cart, removeFromCart } = useCart();

  const total = cart.reduce((sum, item) => sum + parseFloat(item.price || 0), 0).toFixed(2);

  return (
    <View style={tw`flex-1 bg-gray-800`}>
      <Appbar.Header style={tw`bg-green-800`}>
        <Appbar.Content 
          title="Estoque Tático" 
          titleStyle={tw`text-gray-100 font-bold`}
        />
      </Appbar.Header>

      <ScrollView contentContainerStyle={tw`p-4`}>
        {cart.length === 0 ? (
          <Text style={tw`text-center text-gray-400 mt-10 italic text-lg`}>
            Seu estoque está vazio.
          </Text>
        ) : (
          cart.map(item => (
            <View
              key={item.cartItemId}
              style={tw`bg-gray-700 rounded-lg border border-green-600 p-4 mb-4`}
            >
              <List.Item
                title={item.name}
                titleStyle={tw`text-green-400 font-semibold text-lg`}
                description={`R$ ${item.price}`}
                descriptionStyle={tw`text-gray-300`}
                right={() => (
                  <View style={tw`flex-row items-center`}>
                    <Button
                      mode="outlined"
                      onPress={() => removeFromCart(item.cartItemId)}
                      compact
                      style={tw`mr-2 border-green-500`}
                      labelStyle={tw`text-green-400`}
                    >
                      Remover
                    </Button>
                    <IconButton
                      icon="delete"
                      size={24}
                      onPress={() => removeFromCart(item.cartItemId)}
                      color="#16a34a"
                    />
                  </View>
                )}
              />
            </View>
          ))
        )}

        {cart.length > 0 && (
          <Text style={tw`text-right font-bold text-xl mt-4 text-green-500`}>
            Total: R$ {total}
          </Text>
        )}

        <Button
          mode="contained"
          style={tw`mt-6 bg-green-700 rounded-full shadow-lg`}
          onPress={() => navigation.navigate('FinalizarCompra')}
          disabled={cart.length === 0}
          labelStyle={tw`text-white font-semibold`}
        >
          Finalizar Missão
        </Button>
      </ScrollView>
    </View>
  );
}