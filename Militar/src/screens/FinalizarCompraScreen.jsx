...
export default function FinalizarCompraScreen({ navigation }) {
  ...
  if (!cliente) {
    return (
      <View style={tw`flex-1 bg-green-800 justify-center items-center p-6`}>
        <Text style={tw`mb-4 text-center text-white text-lg font-semibold`}>
          Você precisa cadastrar um cliente antes de finalizar a compra.
        </Text>
        <Button mode="contained" onPress={() => navigation.navigate('CadastroCliente')} style={tw`bg-gray-700 px-6 py-2 rounded-full`}>
          Cadastrar Cliente
        </Button>
      </View>
    );
  ...
  return (
    <View style={tw`flex-1 bg-gray-100`}>
      ...
      <ScrollView contentContainerStyle={tw`p-6`}>
        <Text style={tw`text-gray-800 text-xl font-bold mb-4`}>Cliente</Text>
        <View style={tw`bg-white p-4 rounded-lg shadow-md mb-6`}>
          <Text style={tw`text-gray-800 mb-1`}>Nome: {cliente.nome}</Text>
          <Text style={tw`text-gray-800 mb-1`}>Email: {cliente.email}</Text>
          <Text style={tw`text-gray-800 mb-1`}>Telefone: {cliente.telefone}</Text>
          <Text style={tw`text-gray-800 mb-1`}>CPF: {cliente.cpf}</Text>
          <Text style={tw`text-gray-800`}>Endereço: {cliente.endereco}</Text>
        </View>

        <Text style={tw`text-green-800 text-xl font-bold mb-4`}>Produtos</Text>
        <View style={tw`bg-white p-4 rounded-lg shadow-md mb-6`}>
          {cart.map(item => (
            <List.Item
              key={item.cartItemId}
              ...
              descriptionStyle={tw`text-gray-600`}
            />
          ))}
          <Text style={tw`text-right font-bold text-lg text-green-800 mt-4`}>Total: R$ {total}</Text>
        </View>

        <Button
          mode="contained"
          style={tw`bg-green-800 rounded-full py-2 mb-8`}
        ...
        <Text style={tw`text-gray-800 text-xl font-bold mb-4`}>Avaliação da Compra</Text>
            ...
            {({ handleChange, handleSubmit, values, errors, touched }) => (
              <>
                ...
                <MaskInput
                  ...
                  style={[tw`border p-3 mb-4 rounded`, { borderColor: '#4B5563', backgroundColor: 'white' }]}
                />
                ...
                <Button
                  ...
                  style={tw`bg-green-800 rounded-full py-2`}
                >
                  Enviar Avaliação
                </Button>
              </>
        ...
        <List.Section title="Avaliações Recebidas" style={tw`mb-12`}>
          {feedbacks.length === 0 && (
            <Text style={tw`text-green-800 text-center py-6`}>Nenhuma avaliação recebida ainda.</Text>
          )}
          {feedbacks.map(fb => (
            <List.Item
              key={fb.id}
